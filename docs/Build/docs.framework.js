...
var ASM_CONSTS = {
  5283760: function() {return Module.webglContextAttributes.premultipliedAlpha;},  
 5283821: function() {return Module.webglContextAttributes.preserveDrawingBuffer;},  
 5283885: function() {return Module.webglContextAttributes.powerPreference;},  
 5283943: function() {Module['emscripten_get_now_backup'] = performance.now;},  
 5283998: function($0) {performance.now = function() { return $0; };},  
 5284046: function($0) {performance.now = function() { return $0; };},  
 5284094: function() {performance.now = Module['emscripten_get_now_backup'];}
};
  ...
  function _ClearGameRoom(roomIdPtr) {
          ...
          console.log("[ClearGameRoom] Clearing room:", roomId);
          ...
          try {
              ...
              // units, intents, cycle をクリア (players/ は保持)
              const updates = {};
              updates[roomId + '/units'] = null;
              updates[roomId + '/intents'] = null;
              updates[roomId + '/cycle'] = null; // ★サイクルもクリア
              
              window.database.ref().update(updates).then(() => {
                  console.log("[ClearGameRoom] Room data cleared successfully");
              }).catch(error => {
                  console.error("[ClearGameRoom] Failed to clear room data:", error);
              });
              
          } catch (e) {
              console.error("[ClearGameRoom] Error:", e);
          }
      }

  function _ClearHost(roomIdPtr) {
          const roomId = UTF8ToString(roomIdPtr);
          
          console.log("[ClearHost] Clearing host for room:", roomId);
          
          try {
              if (!window.database) {
                  console.error("[ClearHost] Firebase database not initialized");
                  return;
              }
              ...
              window.database.ref(roomId + '/host').remove().then(function() {
                  console.log("[ClearHost] Host cleared successfully");
              }).catch(function(error) {
                  console.error("[ClearHost] Failed to clear host:", error);
              });
              
          } catch (e) {
              console.error("[ClearHost] Error:", e);
          }
      }

  function _ClearPlayerPresence(roomIdPtr, playerNumber) {
          const roomId = UTF8ToString(roomIdPtr);
          
          console.log("[ClearPlayerPresence] Called:", playerNumber);
          
          try {
              if (!window.database) {
                  console.error("[ClearPlayerPresence] Firebase database not initialized");
                  return;
              }
  
              const playerRef = window.database.ref(roomId + '/players/' + playerNumber);
              playerRef.remove().then(() => {
                  console.log("[ClearPlayerPresence] Success:", playerNumber);
              }).catch(error => {
                  console.error("[ClearPlayerPresence] Failed:", error);
              });
              
          } catch (e) {
              console.error("[ClearPlayerPresence] Error:", e);
          }
      }

  function _ClearResetCommand(roomIdPtr) {
          const roomId = UTF8ToString(roomIdPtr);
          console.log("[ClearResetCommand] Clearing reset command");
          try {
              if (!window.database) {
                  console.error("[ClearResetCommand] Firebase database not initialized");
                  return;
              }
              window.database.ref(roomId + '/control/reset').remove().then(function() {
                  console.log("[ClearResetCommand] Reset command cleared");
              }).catch(function(error) {
                  console.error("[ClearResetCommand] Failed to clear reset:", error);
              });
          } catch (e) {
              console.error("[ClearResetCommand] Error:", e);
          }
      }

  function _DeleteUnitData(path) {
          ...
          try {
              ...
              window.database.ref(pathStr).remove();
              console.log("Unit data deleted from:", pathStr);
          } catch (e) {
              console.error("Failed to delete unit data:", e);
          }
      }
  ...
  function _InitFirebase(apiKey, authDomain, databaseURL, projectId, storageBucket, messagingSenderId, appId) {
          const firebaseConfig = {
              apiKey: UTF8ToString(apiKey),
              authDomain: UTF8ToString(authDomain),
              databaseURL: UTF8ToString(databaseURL),
  ...
  function _ListenToControl(path) {
          const pathStr = UTF8ToString(path);
  
          console.log("[ListenToControl] Called with path:", pathStr);
  
          try {
              if (!window.database) {
                  console.error("[ListenToControl] Firebase database not initialized");
                  return;
              }
  
              window.database.ref(pathStr).on('value', (snapshot) => {
                  const data = snapshot.val() || {};
                  // 優先して data.reset を送る（reset ノードを使う設計）
                  const payload = data && data.reset ? data.reset : data;
                  let json = "{}";
                  try {
                      json = JSON.stringify(payload);
                  } catch (e) {
                      console.error("[ListenToControl] Failed to stringify control data:", e);
                  }
  
                  try {
                      const target = window.unityFirebaseCallbackObjectName || 'NetworkManager';
                      SendMessage(target, 'OnControlDataReceived', json);
                      console.log("[ListenToControl] Control sent to Unity (len=" + json.length + ")");
                  } catch (e) {
                      console.error("[ListenToControl] Failed to send control to Unity:", e);
                  }
              });
  
              console.log("[ListenToControl] Listening to control at:", pathStr);
          } catch (e) {
              console.error("[ListenToControl] Error:", e);
          }
      }

  function _ListenToCycle(path) {
          const pathStr = UTF8ToString(path);
  
          console.log("[ListenToCycle] Called with path:", pathStr);
  
          try {
              if (!window.database) {
                  console.error("[ListenToCycle] Firebase database not initialized");
                  return;
              }
  
              window.database.ref(pathStr).on('value', (snapshot) => {
                  const data = snapshot.val();
                  const cycleData = data || { count: 0 };
                  
                  try {
                      const target = window.unityFirebaseCallbackObjectName || 'NetworkManager';
                      SendMessage(target, 'OnCycleDataReceived', JSON.stringify(cycleData));
                      console.log("[ListenToCycle] Cycle data received:", cycleData.count);
                  } catch (e) {
                      console.error("[ListenToCycle] Failed to send cycle data to Unity:", e);
                  }
              });
  
              console.log("[ListenToCycle] Listening to cycle at:", pathStr);
          } catch (e) {
              console.error("[ListenToCycle] Error:", e);
          }
      }

  function _ListenToIntents(path) {
          const pathStr = UTF8ToString(path);
  
          console.log("[ListenToIntents] Called with path:", pathStr);
  
          try {
              if (!window.database) {
                  console.error("[ListenToIntents] Firebase database not initialized");
                  return;
              }
              ...
              window.database.ref(pathStr).on('value', (snapshot) => {
                  ...
                  if (data) {
                      Object.keys(data).forEach(cycleKey => {
                          const cycleGroup = data[cycleKey];
                          const cycle = parseInt(cycleKey);
                          if (!cycleGroup) return;
                          Object.keys(cycleGroup).forEach(unitId => {
                              const item = cycleGroup[unitId];
                              if (item) {
                                  const intentObj = Object.assign({}, item);
                                  intentObj.cycle = cycle;
                                  intentObj.unitId = unitId;
                  ...
                  try {
                      const target = window.unityFirebaseCallbackObjectName || 'NetworkManager';
                      SendMessage(target, 'OnIntentDataReceived', JSON.stringify(intentsArray));
                      console.log("[ListenToIntents] Intent data received:", intentsArray.length, "intents");
                  } catch (e) {
                      console.error("[ListenToIntents] Failed to send intents to Unity:", e);
                  }
              });
  
              console.log("[ListenToIntents] Listening to intents at:", pathStr);
          } catch (e) {
              console.error("[ListenToIntents] Error:", e);
          }
      }

  function _ListenToSnapshot(path) {
          const pathStr = UTF8ToString(path);
  
          console.log("[ListenToSnapshot] Called with path:", pathStr);
  
          try {
              if (!window.database) {
                  console.error("[ListenToSnapshot] Firebase database not initialized");
                  return;
              }
              ...
              window.database.ref(pathStr).on('value', (snapshot) => {
                  ...
                  try {
                      ...
                      console.error("[ListenToSnapshot] Failed to stringify snapshot data:", e);
                  }
  
                  try {
                      const target = window.unityFirebaseCallbackObjectName || 'NetworkManager';
                      SendMessage(target, 'OnSnapshotDataReceived', json);
                      console.log("[ListenToSnapshot] Snapshot sent to Unity (len=" + json.length + ")");
                  } catch (e) {
                      console.error("[ListenToSnapshot] Failed to send snapshot to Unity:", e);
                  }
              });
  
              console.log("[ListenToSnapshot] Listening to snapshot at:", pathStr);
          } catch (e) {
              console.error("[ListenToSnapshot] Error:", e);
          }
      }

  function _ListenToUnits(path) {
          const pathStr = UTF8ToString(path);
          ...
          try {
              ...
              window.database.ref(pathStr).on('value', (snapshot) => {
                  const data = snapshot.val();
                  const unitsArray = data ? Object.values(data) : [];
                  try {
                      const target = window.unityFirebaseCallbackObjectName || 'NetworkManager';
                      SendMessage(target, 'OnUnitDataReceived', JSON.stringify(unitsArray));
                      console.log("Unit data received:", unitsArray.length, "units");
                  } catch (e) {
                      console.error("Failed to send unit data to Unity:", e);
                  }
              });
              console.log("Listening to units at:", pathStr);
          } catch (e) {
              console.error("Failed to listen to units:", e);
          }
      }
  ...
  function _SetPlayerPresence(roomIdPtr, playerNumber, playerNamePtr) {
      ...
      console.log("✅ [SetPlayerPresence] Called:", playerNumber, playerName);
      
      try {
          if (!window.database) {
              console.error("❌ [SetPlayerPresence] Firebase database not initialized");
              return;
          }
          ...
          playerRef.update({
              ...
              console.log("✅ [SetPlayerPresence] Updated:", playerNumber);
          }).catch(function(error) {
              console.error("❌ [SetPlayerPresence] Failed:", error);
          });
          
      } catch (e) {
          console.error("❌ [SetPlayerPresence] Error:", e);
      }
  }

  function _SetUnityCallbackObjectName(namePtr) {
          try {
              window.unityFirebaseCallbackObjectName = UTF8ToString(namePtr);
              console.log("Set unity callback object name:", window.unityFirebaseCallbackObjectName);
          } catch (e) {
              console.error("SetUnityCallbackObjectName error:", e);
          }
      }

  function _StartHostMonitoring(roomIdPtr) {
          const roomId = UTF8ToString(roomIdPtr);
          
          console.log("[StartHostMonitoring] Starting for room:", roomId);
          
          try {
              if (!window.database) {
                  console.error("[StartHostMonitoring] Firebase database not initialized");
                  return;
              }
              ...
              console.log("[StartHostMonitoring] Monitoring started");
              
          } catch (e) {
              console.error("[StartHostMonitoring] Error:", e);
          }
      }

  function _StartListenersAfterInit(roomIdPtr) {
          const roomId = UTF8ToString(roomIdPtr);
          
          console.log("⏳ [StartListenersAfterInit] Waiting for initialization...");
          
          // ★初期化完了を待つ
          const waitForInit = window.initializationPromise || Promise.resolve();
          
          waitForInit.then(function() {
              console.log("✅ [StartListenersAfterInit] Initialization complete, starting listeners");
              
              // ★少し待ってからリスナーを開始（初期化が確実に反映されるまで）
              setTimeout(function() {
                  const target = window.unityFirebaseCallbackObjectName || 'NetworkManager';
                  SendMessage(target, 'OnReadyToStartListeners', '');
              }, 500);  // 500ms 待機
              
          }).catch(function(error) {
              console.error("❌ [StartListenersAfterInit] Error:", error);
          });
      }

  function _StopHostHeartbeat() {
          console.log("[StopHostHeartbeat] Stopping heartbeat");
          if (window.hostHeartbeatInterval) {
              clearInterval(window.hostHeartbeatInterval);
              window.hostHeartbeatInterval = null;
          }
  ...
  function _SubmitIntentJs(path, jsonData) {
          const pathStr = UTF8ToString(path);
          const dataStr = UTF8ToString(jsonData);
          ...
          try {
              ...
              window.database.ref(pathStr).set(data);
              console.log("Intent written to:", pathStr);
          } catch (e) {
              console.error("Failed to submit intent:", e);
          }
      }

  function _TryBecomeHost(roomIdPtr, playerNumber) {
          const roomId = UTF8ToString(roomIdPtr);
          
          // ★追加: playerNumber が 0 または負数なら拒否
          if (playerNumber <= 0) {
              console.warn("[TryBecomeHost] Invalid playerNumber:", playerNumber, "- aborting");
              return;
          }
          
          console.log("[TryBecomeHost] Called with roomId:", roomId, "playerNumber:", playerNumber);
          
          try {
              if (!window.database) {
                  console.error("[TryBecomeHost] Firebase database not initialized");
                  return;
              }
              ...
              // ★重要: Promise を作成して保存
              window.initializationPromise = playersRef.once('value').then(function(playersSnapshot) {
                  const players = playersSnapshot.val() || {};
                  const playerCount = Object.keys(players).length;
                  
                  console.log("👥 [TryBecomeHost] Current player count:", playerCount);
                  ...
                  return hostRef.once('value').then(function(hostSnapshot) {
                      ...
                      if (shouldInitialize) {
                          ...
                          updates[roomId + '/snapshot'] = null;
                          updates[roomId + '/host'] = null;
                          updates[roomId + '/control'] = null;  // ★control も追加
                          
                          return window.database.ref().update(updates).then(function() {
                              console.log("✅ [TryBecomeHost] Server initialization complete");
                              return { initialized: true };
                          });
                      } else {
                          console.log("👥 [TryBecomeHost] Other players or host exists, skipping initialization");
                          return { initialized: false };
                      }
                  });
                  
              }).then(function(initResult) {
                  console.log("📊 [TryBecomeHost] Initialization result:", initResult);
                  ...
                  if (result.committed) {
                      console.log("✅ [TryBecomeHost] Successfully became host:", playerNumber);
                      ...
                      hostRef.once('value').then(function(snapshot) {
                          const host = snapshot.val();
                          console.log("ℹ️ [TryBecomeHost] Another host exists:", host);
                          const target = window.unityFirebaseCallbackObjectName || 'NetworkManager';
                          SendMessage(target, 'OnHostStatusReceived', JSON.stringify({
                              isHost: false,
                  ...
                  console.error("❌ [TryBecomeHost] Error:", error);
              });
              
          } catch (e) {
              console.error("❌ [TryBecomeHost] Error:", e);
          }
      }

  function _UpdateCycleCount(pathPtr, cycleCount) {
          var path = UTF8ToString(pathPtr);
          console.log("[jslib] UpdateCycleCount: path=" + path + ", count=" + cycleCount);
          ...
          dbRef.set(cycleData)
              .then(function() {
                  console.log("[jslib] ✓ Cycle count updated successfully: " + cycleCount);
              })
              .catch(function(error) {
                  console.error("[jslib] ❌ UpdateCycleCount failed: " + error.message);
              });
      }
  ...
  function _WriteUnitData(path, jsonData) {
          ...
          try {
              ...
              const data = JSON.parse(dataStr);
              window.database.ref(pathStr).set(data);
              console.log("Unit data written to:", pathStr);
          } catch (e) {
              console.error("Failed to write unit data:", e);
          }
      }
...
var asmLibraryArg = {
  ...
  "ClearResetCommand": _ClearResetCommand,
  "DeleteUnitData": _DeleteUnitData,
  ...
  "ListenToControl": _ListenToControl,
  "ListenToCycle": _ListenToCycle,
  "ListenToIntents": _ListenToIntents,
  "ListenToSnapshot": _ListenToSnapshot,
  "ListenToUnits": _ListenToUnits,
  "RequestPlayerID": _RequestPlayerID,
  "SetPlayerPresence": _SetPlayerPresence,
  "SetUnityCallbackObjectName": _SetUnityCallbackObjectName,
  "StartHostMonitoring": _StartHostMonitoring,
  "StartListenersAfterInit": _StartListenersAfterInit,
  "StopHostHeartbeat": _StopHostHeartbeat,
  "SubmitIntentJs": _SubmitIntentJs,
  "TryBecomeHost": _TryBecomeHost,
  "UpdateCycleCount": _UpdateCycleCount,
  "WriteResetCommand": _WriteResetCommand,
  "WriteUnitData": _WriteUnitData,
